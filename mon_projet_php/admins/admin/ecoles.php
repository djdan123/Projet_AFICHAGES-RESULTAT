...
if (isset($_GET['supprimer'])) {
    ...
    $conn->query("DELETE FROM ecole WHERE codeEcole=$id");
...
$result = $conn->query("SELECT * FROM ecole ORDER BY codeEcole DESC");
?>
        ...
        <tbody>
            <?php if ($result && $result->num_rows > 0): ?>
                ...
                <tr>
                    ...
                    <td><?= $row['code_DCE'] ?></td>
                    <td>
                       <a href="?supprimer=<?= $row['codeEcole'] ?>" onclick="return confirm('Supprimer cette école ?');">Supprimer</a>
                    </td>
                </tr>
                <?php endwhile; ?>
            <?php else: ?>
